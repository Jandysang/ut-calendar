...
export default class DateInfo {
    constructor(config, { order = 0, locale, isFullDay } = {}) {
      ...
      if (config.on) {
        const or = (_.isArrayLikeObject(config.on) ? config.on : [config.on])
          .map(o => {
            if (_.isFunction(o)) return o;
    ...
    static get patterns() {
      return {
        ...
        days: {
          validate: days => (_.isArrayLikeObject(days) ? days : [parseInt(days, 10)]),
          test: (day, days) =>
            days.includes(day.day) || days.includes(-day.dayFromEnd),
        },
        weekdays: {
          validate: weekdays =>
          _.isArrayLikeObject(weekdays) ? weekdays : [parseInt(weekdays, 10)],
        ...
        ordinalWeekdays: {
          validate: ordinalWeekdays =>
            Object.keys(ordinalWeekdays).reduce((obj, ck) => {
              ...
              obj[ck] = _.isArrayLikeObject(weekdays) ? weekdays : [parseInt(weekdays, 10)];
              return obj;
            }, {}),
        ...
        weeks: {
          validate: weeks => (_.isArrayLikeObject(weeks) ? weeks : [parseInt(weeks, 10)]),
          test: (day, weeks) =>
            weeks.includes(day.week) || weeks.includes(-day.weekFromEnd),
        },
        months: {
          validate: months => (_.isArrayLikeObject(months) ? months : [parseInt(months, 10)]),
          test: (day, months) => months.includes(day.month),
        },
        years: {
          validate: years => (_.isArrayLikeObject(years) ? years : [parseInt(years, 10)]),
    ...
    static testConfig(config, day, dateInfo) {
      if (_.isFunction(config)) return config(day);
      if (isObject(config)) {
    ...
    iterateDatesInRange({ start, end }, fn) {
      if (!start || !end || !_.isFunction(fn)) return null;
